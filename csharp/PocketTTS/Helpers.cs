...
{
    public static void WriteWav(float[] rawAudio, uint sampleRate, string outputPath)
    {
        ...
        writer.Write(wavHeaderSize + rawAudio.Length * bytesPerSample - 8); //file-size (equals file-size - 8). Size of the overall file - 8 bytes
        ...
        writer.Write(rawAudio.Length * numChannels * bytesPerSample); //data length
